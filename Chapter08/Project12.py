...
while lagiPilihan == 'y':
    ...
    if(pilihan == 'A'):
        ...
        delData = input('Masukkan nama buah yang ingin dihapus : ')
        if(delData not in buah):
            print('Buah {0} tidak ada didalam daftar')
        else:
            del(buah[delData])
            print('Buah {0} berhasil dihapus dari daftar'.format(delData))
            showData(buah)
